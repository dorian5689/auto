...
class DrissionElement(BaseElement):
    ...
    def prev(self, filter_loc='', index=1, timeout=None, ele_only=True):
        ...
        if isinstance(filter_loc, int):
            index = filter_loc
            filter_loc = ''
        nodes = self._get_relatives(index, filter_loc, 'preceding', timeout=timeout, ele_only=ele_only)
        if nodes:
            return nodes[-1]
        if Settings.raise_when_ele_not_found:
    ...
    def next(self, filter_loc='', index=1, timeout=None, ele_only=True):
        ...
        if isinstance(filter_loc, int):
            index = filter_loc
            filter_loc = ''
        nodes = self._get_relatives(index, filter_loc, 'following', timeout=timeout, ele_only=ele_only)
        if nodes:
            return nodes[0]
        if Settings.raise_when_ele_not_found:
    ...
    def before(self, filter_loc='', index=1, timeout=None, ele_only=True):
        ...
        if isinstance(filter_loc, int):
            index = filter_loc
            filter_loc = ''
        nodes = self._get_relatives(index, filter_loc, 'preceding', False, timeout=timeout, ele_only=ele_only)
        if nodes:
            return nodes[-1]
        if Settings.raise_when_ele_not_found:
    ...
    def after(self, filter_loc='', index=1, timeout=None, ele_only=True):
        ...
        if isinstance(filter_loc, int):
            index = filter_loc
            filter_loc = ''
        nodes = self._get_relatives(index, filter_loc, 'following', False, timeout, ele_only=ele_only)
        if nodes:
            return nodes[0]
        if Settings.raise_when_ele_not_found:
